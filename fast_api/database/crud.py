...
def create_deal(category: schemas.DealCreate, db: Session, deal_id: int):
    db_user = models.Deal(**category.dict(), id_deal=deal_id)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def create_user(category: schemas.UserCreate, db: Session, user_id: int):
    db_user = models.User(**category.dict(), user_id=user_id)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    return db_user
...
def create_notification(category: schemas.NotificationsCreate, db: Session, owner_id: int):
    db_user = models.Notifications(**category.dict(), owner_id=owner_id)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
